...
discriminator_loss_fn = torch.nn.MSELoss()
...
for epoch in range(0, NUM_EPOCHS):
    ...
    for batch in range(0, int(TRAIN_DATA_SIZE/BATCH_SIZE)):
        ...
        fake_images = [generator(torch.rand(10)) for i in range(0, BATCH_SIZE)]
        input_tensor = torch.stack(real_images+fake_images)
        discriminator_opt.zero_grad()
        generator_opt.zero_grad()
        decision = discriminator(input_tensor.float())
        discriminator_correct = torch.tensor([1]*BATCH_SIZE+[0]*BATCH_SIZE)
        discriminator_train_loss = discriminator_loss_fn(decision, discriminator_correct.float())
        discriminator_train_loss.backward()
        discriminator_opt.step()
        discriminator_batch_loss += discriminator_train_loss.data.item()
        decision_list = decision.to_list()
        mapped_decisions = torch.tensor([np.random.choice([0, 1], 1, p=[1-i, i]) for i in decision_list])
        generator_pred = torch.abs(mapped_decisions-discriminator_correct)
        generator_label = torch.ones(BATCH_SIZE)
        generator_train_loss = generator_loss_fn(generator_pred, generator_label.float())
